...
function MessagePreview({ message }: { message: string }) {
  ...
  return (
    <div className="flex items-start gap-2">
      <svg className="mt-0.5 h-3.5 w-3.5 shrink-0 text-zinc-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="min-w-0">
        <p className={`text-xs italic text-zinc-400 leading-relaxed ${expanded ? "" : "truncate"}`}>
          &quot;{message}&quot;
        </p>
        {isLong && (
          <span
            ...
            className="mt-0.5 inline-block cursor-pointer text-[10px] font-medium text-[#7A0022]/70 hover:text-[#7A0022] transition"
          >
...
export default function AdminPage() {
  ...
  const [cardGuest, setCardGuest] = useState<RsvpRow | null>(null);
  ...
  useEffect(() => {
    load();
    localStorage.setItem("zh_admin", "1");
  ...
  return (
    <main className="min-h-screen bg-[#FBF7F2] text-zinc-800">
      ...
      <div className="mx-auto max-w-7xl px-4 pb-16 pt-10 sm:px-6 sm:pt-14">
        ...
        <div className="mb-8 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link href="/admin/groups" className="inline-flex items-center gap-1.5 rounded-full bg-[#7A0022] px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-[#64001C] transition">
              <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
              Groups
            </Link>
            <Link href="/admin/calendar" className="inline-flex items-center gap-1.5 rounded-full bg-[#7A0022] px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-[#64001C] transition">
              <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /><path strokeLinecap="round" strokeLinejoin="round" d="M9 13h.01M12 13h.01M15 13h.01M9 16h.01M12 16h.01M15 16h.01" /></svg>
              Calendar
            </Link>
            ...
            <Link href="/admin/contact" className="inline-flex items-center gap-1.5 rounded-full bg-[#7A0022] px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-[#64001C] transition">
              <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
              Contact
            </Link>
            <Link href="/admin/changelog" className="inline-flex items-center gap-1.5 rounded-full bg-[#7A0022] px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-[#64001C] transition">
            ...
            {/* <Link href="/admin/likes" className="inline-flex items-center gap-1.5 rounded-full bg-[#7A0022] px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-[#64001C] transition">
                ...
                >
                  ...
                  <button onClick={() => setExpandedId(isExpanded ? null : r.id)} className="w-full text-left">
                    <div className="flex flex-col gap-4 p-5">
                      ...
                      {r.message && r.message.trim().length > 0 && (
                        <MessagePreview message={r.message.trim()} />
                      )}
                  ...
                  {isExpanded && (
                    <div className="border-t border-black/5 bg-zinc-50/70 px-5 py-4 flex flex-col gap-3">
                      ...
                      {/* Delete + Save Card + ID row */}
                      <div className="flex items-center justify-between pt-1">
                        <p className="text-[10px] font-mono text-zinc-300 select-none">#{r.id.slice(0, 8)}</p>
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => setCardGuest(r)}
                            className="inline-flex items-center gap-1 rounded-xl border border-[#7A0022]/20 bg-[#7A0022]/5 px-3 py-2 text-xs font-semibold text-[#7A0022] hover:bg-[#7A0022]/10 transition"
                          >
      ...
      {/* ── Guest Card Modal ── */}
      {cardGuest && <GuestCardModal guest={cardGuest} onClose={() => setCardGuest(null)} />}
    </main>
  );
}
...
function GuestCardModal({ guest, onClose }: { guest: RsvpRow; onClose: () => void }) {
  ...
  const guests = String(guest.guests ?? 1);
  const adults = String(guest.adults ?? 1);
  const kids = String(guest.kids ?? 0);
  const guestOf = guest.guest_of ?? "";
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/70 backdrop-blur-sm px-5 py-8 overflow-y-auto">
      <div className="absolute inset-0" onClick={imageUrl ? undefined : onClose} />
      ...
      {!imageUrl ? (
        <div className="relative flex flex-col items-center gap-5">
          ...
          <div
            ref={cardRef}
            className="w-85 overflow-hidden rounded-[28px] bg-[#FBF7F2] shadow-[0_20px_70px_rgba(0,0,0,0.25)]"
          ...
          >
            <div className="bg-[#7A0022] px-7 py-6 text-center">
              ...
              <p className="mt-2 text-3xl text-white" style={{ fontFamily: "'Great Vibes', Georgia, cursive", letterSpacing: "0.02em" }}>
                Zhafirin &amp; Ayuni
              </p>
            ...
            <div className="mx-7 mt-4 grid grid-cols-2 gap-2.5">
              {[["Session", session || "—"], ["Time", time || "—"], ["Guests", `${guests} pax`], ["Adults / Kids", `${adults} / ${kids}`]].map(([label, value]) => (
                <div key={label} className="rounded-xl border border-[#7A0022]/10 bg-white/70 px-3 py-2.5 text-center">
          ...
          <div className="flex gap-3">
            <button
              onClick={saveImage}
              disabled={saving}
              className="inline-flex items-center gap-2 rounded-full bg-[#7A0022] px-6 py-3 text-sm font-medium text-white shadow-lg hover:bg-[#64001C] disabled:opacity-60 transition"
            >
              {saving ? (
                <>
            ...
            <button
              onClick={onClose}
              className="rounded-full border border-white/20 bg-white/10 px-5 py-3 text-sm text-white hover:bg-white/20 transition"
            >
              Close
            </button>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-white font-medium">
              {/iPhone|iPad|iPod/i.test(typeof navigator !== "undefined" ? navigator.userAgent : "")
                ...
                : "Long press or right-click → Save image"}
            </p>
          </div>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={imageUrl} alt="RSVP card" className="max-h-[65vh] w-auto rounded-[20px] shadow-2xl" />
          <button
            onClick={() => { setImageUrl(null); onClose(); }}
            className="rounded-full border border-white/20 bg-white/10 px-5 py-2.5 text-sm text-white hover:bg-white/20 transition"
          >
            Close
          </button>
        </div>
