        ...
        >
          <Card>
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:justify-center">
              <Link
                href="/our-story"
                className="inline-flex w-full items-center justify-center rounded-full bg-[#7A0022] px-5 py-2.5 text-sm font-medium text-white transition hover:bg-[#64001C]"
              >
                Continue reading →
              </Link>
            </div>
...
function FloralSVG({ side }: { side: "left" | "right" }) {
  ...
  return (
    ...
    >
      ...
      <path d="M90 150c40-30 85-40 120-20-45 35-80 55-120 20Z" fill="#3E6B54" opacity="0.55" />
      <path d="M150 165c55-40 105-55 150-30-55 45-105 70-150 30Z" fill="#2F5B45" opacity="0.45" />
...
function SectionHeader({
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <p className="text-[11px] font-semibold uppercase tracking-[0.34em] text-[#7A0022]/80">{label}</p>
...
function PrimaryButton({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="inline-flex items-center justify-center rounded-full bg-[#7A0022] px-6 py-3 text-sm font-medium text-white shadow-[0_12px_26px_rgba(122,0,34,0.22)] hover:bg-[#64001C]">
      {children}
    </Link>
  );
}

function SecondaryButton({ href, children, icon }: { href: string; children: React.ReactNode; icon?: React.ReactNode }) {
  return (
    <a href={href} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full border border-black/10 bg-white/85 px-6 py-3 text-sm font-medium text-zinc-900 backdrop-blur hover:bg-white">
      {children}
      {icon}
    </a>
  );
